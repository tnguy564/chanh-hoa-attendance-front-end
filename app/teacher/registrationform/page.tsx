...
export default function StudentRegistrationForm() {
  ...
  const handlePhotoCapture = async (images: string[]) => {
    ...
    try {
      ...
      if (data.success) {
        setStatus(`✅ Member registered successfully! ID: ${formData.studentId}`);
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Header */}
      <header className="bg-[#212121] border-b-2 border-slate-200 shadow-lg relative z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-medium">
                Step {step} of 2: {step === 1 ? "Member Details" : "Photo Capture"}
              </p>
          ...
          {step === 1 ? (
            <div className="bg-white/90 backdrop-blur-lg rounded-2xl p-6 border-2 border-slate-200 shadow-2xl">
              ...
              <div className="text-center mb-6">
                <div className="inline-flex items-center gap-2 px-4 py-2 border-2 border-blue-200 rounded-xl mb-3">
                  <User className="w-5 h-5 text-blue-600" />
                  <span className="text-blue-700 font-semibold">Member Information</span>
                </div>
                <h2 className="text-3xl font-bold text-[#274e13]  mb-2">Register New Member</h2>
                <p className="text-slate-600 text-base leading-relaxed">
                  Fill in all required member details to create a comprehensive profile
                </p>
              </div>
              ...
              <form onSubmit={handleFormSubmit}>
                <div className="grid grid-cols-1 gap-6">
                  ...
                  <div className="rounded-xl p-6 border-2 border-blue-200 shadow-lg hover:shadow-xl transition-all duration-300">
                    <div className="flex items-center gap-3 mb-5">
                      <h3 className="text-xl font-bold text-slate-900">Personal Details</h3>
                    </div>
                    <div className="space-y-4">
                      <InputField name="studentName" value={formData.studentName} onChange={handleInputChange} icon={User} label="Full Name *" placeholder="Enter member's full name" />
                      <InputField name="studentId" value={formData.studentId} onChange={handleInputChange} icon={IdCard} label="Member ID *" placeholder="Enter unique member ID" />
                      <InputField name="buddhaName" value={formData.buddhaName} onChange={handleInputChange} icon={IdCard} label="Buddha Name *" placeholder="Enter member's buddha name" />
                      <div className="space-y-2">
                      <label className="text-slate-900 text-sm font-bold flex items-center gap-2">                    
                        Role *
                </label>
                <div className="relative group">
                    {/* Left Icon (Matches your InputField style) */}
                ...
                className="w-full pl-11 pr-10 py-3 rounded-lg bg-white/60 border border-gray-200 text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 appearance-none cursor-pointer"
                >
                ...
                <option value="staff">Huynh Trưởng</option>
                    ...
                    <InputField name="email" value={formData.email} onChange={handleInputChange} icon={Mail} label="Email Address *" placeholder="Enter member's email" type="email" />
            ...
            <div className="bg-white/90 backdrop-blur-lg rounded-2xl p-6 border-2 border-slate-200 shadow-2xl">
              <div className="text-center mb-6">
                <div className="inline-flex items-center gap-2 px-4 py-2 border-2 border-purple-200 rounded-xl mb-3">
